...
const SKELETON_FEED = [1, 2, 3, 4, 5] as const;
    ...
    <div className="rounded-xl border border-zinc-800 bg-zinc-900 p-5 fade-in">
      ...
      {loading ? (
        <div className="space-y-3">
          {SKELETON_BARS.map((n) => (
            <div key={n} className="h-6 animate-pulse rounded bg-zinc-800" />
          ))}
        </div>
      ) : entries.length === 0 ? (
...
export function OverviewContent() {
  ...
  // Track consecutive auth failures so we stop the interval and avoid flooding
  ...
  const fetchData = useCallback(async () => {
    ...
    try {
      ...
      if (!res.ok) {
        ...
        // Count auth errors so the interval can back off
