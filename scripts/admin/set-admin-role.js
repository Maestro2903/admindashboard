...
const VALID_ROLES = ['viewer', 'manager', 'superadmin'];
...
async function setAdminRole(email, role) {
  try {
    ...
    console.log(`✅ Set adminRole to '${role}' for ${email}`);
...
if (!email || !role) {
  console.error('Usage: node scripts/admin/set-admin-role.js <email> <role>');
  console.error('Roles: viewer | manager | superadmin');
  console.error('Example: node scripts/admin/set-admin-role.js user@example.com viewer');
  process.exit(1);
}
